#pragma	title("Printer Driver  --  Version 3  --  (Fonts.C)")
...
if ( (i	= (INT)LONGFROMMR(WinSendMsg(hwndFont, LM_QUERYSELECTION,
				     0L, 0L))) != LIT_NONE )
   {
		       /* Get the font match number of the font		*/
		       /* selected					*/

   pfsel->lMatch = (LONG)LONGFROMMR(WinSendMsg(hwndFont, LM_QUERYITEMHANDLE,
					       MPFROMSHORT(i), 0L));
